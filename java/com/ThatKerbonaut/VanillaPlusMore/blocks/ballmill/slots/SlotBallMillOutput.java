...
public class SlotBallMillOutput extends Slot{
	private final EntityPlayer player;
	private int removeCount;
	public SlotBallMillOutput(EntityPlayer player, IInventory inventoryIn, int slotIndex, int xPosition, int yPosition) {
	...
	protected void onCrafting(ItemStack stack){
        ...
        if (!this.player.world.isRemote){
            ...
            float f = BallMillRecipes.instance().getCookingExperience(stack);
            if (f == 0.0F){
                i = 0;
            }
            else if (f < 1.0F){
